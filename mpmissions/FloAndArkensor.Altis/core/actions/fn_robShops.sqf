...
_chance = random(100);
if(_chance >= 85) then { hint "Der stille Alarm wurde ausgelöst, du solltest jetzt fliehen!"; [[1,format["ALARM! - Laden: %1 wird ausgeraubt!", _shop]],"life_fnc_broadcast",west,false] spawn life_fnc_MP; };
...
if(_rip) then
{
   ...
   if(_robber getVariable "restrained") exitWith { deleteMarker _marker; _rip = false; _shop switchMove ""; hint "Du wurdest festgenommen!"; 5 cutText ["","PLAIN"]; [[getPlayerUID _robber,name _robber,"57"],"life_fnc_wantedAdd",false,false] spawn life_fnc_MP; };
