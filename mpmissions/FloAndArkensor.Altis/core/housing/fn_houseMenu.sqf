...
if(_curTarget isKindOf "House_F" && playerSide == west) exitWith {
	if((nearestObject [[16750.6,13640.9,0],"Land_Dome_Small_F"]) == _curTarget OR (nearestObject [[16750.6,13640.9,0],"Land_Research_house_V1_F"]) == _curTarget) then {
		
		_Btn1 ctrlSetText "Versiegeln";
...
if(!(_curTarget in life_vehicles) OR isNil {_curTarget getVariable "house_owner"}) then {
	...
	if((typeOf _curTarget) in ["Land_i_Garage_V1_F","Land_i_Garage_V2_F"]) then {
		...
		_Btn4 ctrlSetText "Alarm abstellen";
		_Btn4 buttonSetAction "[life_pInact_curTarget] call life_fnc_HouseAlarmOff; closeDialog 0;";
		_Btn4 ctrlShow true;
		};
