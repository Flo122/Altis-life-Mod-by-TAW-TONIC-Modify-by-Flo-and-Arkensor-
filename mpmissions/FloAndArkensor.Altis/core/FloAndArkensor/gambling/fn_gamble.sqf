...
if(_won == 1) then
{
    ...
    if(_jackpot_won == 1) then
    {
        ...
        [[player, king_gamble_casino_is_illegal select king_gamble_casino_idx],"king_fnc_gambleJackpotWon",false,false] spawn life_fnc_MP;
        if(king_gamble_send_jackpot_win_to_chat == 1) then
        {
            [[0,format["%1 %2",name player, king_gamble_send_jackpot_won_to_chat_message]],"life_fnc_broadcast",nil,false] spawn life_fnc_MP;
        };
...
if(king_gamble_casino_is_illegal select king_gamble_casino_idx == 1) then
{
    ...
    if(_watched_gambling < king_gamble_watched_illegal_gambling_percent) then
    {
        ...
        {
            [[0,format["%1 %2",name player, king_gamble_send_watched_illegal_gambling_to_chat_message]],"life_fnc_broadcast",west,false] spawn life_fnc_MP;
        };
    };
};
