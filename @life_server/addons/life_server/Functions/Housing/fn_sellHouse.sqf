...
if(_houseID == -1) then {
	...
	_query = format["UPDATE houses SET owned='0', pos='[]', security='false' WHERE pid='%1' AND pos='%2' AND owned='1'",_ownerID,_housePos];
	...
	_query = format["UPDATE houses SET owned='0', pos='[]', security='false' WHERE id='%1'",_houseID];
};
